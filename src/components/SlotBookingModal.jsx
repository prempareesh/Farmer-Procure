...
import { X, Calendar, Clock, MapPin, Wheat, Truck, CheckCircle2, ShieldCheck, Download, QrCode } from 'lucide-react';
...
export default function SlotBookingModal({ isOpen, onClose }) {
  ...
  const slots = [
    { time: '08:00 AM - 09:00 AM', congestion: 'Low (12m wait)', status: 'green' },
    { time: '09:00 AM - 10:00 AM', congestion: 'Optimal (18m wait)', status: 'green' },
    { time: '11:00 AM - 12:00 PM', congestion: 'High Congestion (75m wait)', status: 'red' },
    { time: '02:00 PM - 03:00 PM', congestion: 'Moderate (25m wait)', status: 'yellow' },
    { time: '04:00 PM - 05:00 PM', congestion: 'Low (10m wait)', status: 'green' },
  ];
  ...
  const handleConfirm = (e) => {
    ...
    setBookingPass({
      ...
      ...formData,
      qrUrl: `https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=${token}`,
    });
        ...
        <div className="bg-[#2E7D32] text-white p-6 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-white/80">Procure Intelligence • Smart Scheduling</p>
            </div>
          </div>
          <button
            onClick={onClose}
          ...
          {step === 1 ? (
            <form onSubmit={handleConfirm} className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold text-gray-700 uppercase mb-1">Farmer Name</label>
                  <input
                    type="text"
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <input
                          ...
                          onChange={() => setFormData({ ...formData, timeSlot: s.time })}
                          className="accent-[#2E7D32]"
                        />
                        <span className="text-sm font-bold text-gray-900">{s.time}</span>
                      </div>
                      <span className={`text-xs font-semibold px-2.5 py-1 rounded-full ${
                        s.status === 'green' ? 'bg-green-100 text-green-800' :
                        s.status === 'yellow' ? 'bg-yellow-100 text-yellow-800' :
            ...
            <div className="space-y-6 text-center">
              ...
              <div>
                ...
                <h4 className="text-2xl font-black text-gray-900 mt-2">Digital Mandi Token</h4>
                <p className="text-xs text-gray-500 mt-1 font-mono">Token ID: {bookingPass.token}</p>
              </div>
              ...
              <div className="bg-[#FAF8F2] p-5 rounded-2xl border border-[#E8E4D9] text-left space-y-3">
                <div className="grid grid-cols-2 gap-2 text-xs">
                  <div>
                    ...
                    <p className="font-bold text-gray-900">{bookingPass.farmerName}</p>
                  </div>
                  <div>
                    <span className="text-gray-500 font-medium">Phone:</span>
                    <p className="font-bold text-gray-900">{bookingPass.phone}</p>
                  </div>
                  <div className="col-span-2">
                    <span className="text-gray-500 font-medium">Mandi Centre:</span>
                    <p className="font-bold text-gray-900">{bookingPass.mandi}</p>
                  </div>
                  <div>
                    <span className="text-gray-500 font-medium">Crop & Load:</span>
                    <p className="font-bold text-gray-900">{bookingPass.crop} ({bookingPass.quantity})</p>
                  </div>
                  <div>
                    <span className="text-gray-500 font-medium">Scheduled Time:</span>
                    <p className="font-bold text-[#2E7D32]">{bookingPass.timeSlot}</p>
                  </div>
                </div>

                <div className="pt-3 border-t border-gray-200 flex items-center justify-between">
                  <div className="space-y-1">
                    <span className="text-[10px] text-gray-500 font-bold uppercase block">SHA-256 Audit Stamp</span>
