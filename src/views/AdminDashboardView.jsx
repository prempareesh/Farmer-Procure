...
export default function AdminDashboardView() {
  ...
  const pendingFraudAlerts = fraudAlerts.filter(
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-gray-500 font-semibold mt-1">
            Real-time procurement telemetry, predictive bottleneck intervention
            & cryptographic audit supervision
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white rounded-3xl p-5 shadow-md border border-[#E0ECE0] flex items-center gap-4">
          ...
          <div>
            ...
            <h3
              className={`text-xl font-black ${congestionRisk === "HIGH" ? "text-red-700" : "text-[#2E7D32]"}`}
            >
              {congestionRisk} RISK
            </h3>
            <span className="text-[10px] text-gray-500 font-semibold">
                ...
                <tbody className="divide-y divide-gray-100">
                  {mandiCentres.map((centre) => {
                    const pct = Math.round(
                      (centre.todayBooked / centre.todayCapacity) * 100,
                    );
                    return (
                      <tr key={centre.id} className="hover:bg-[#FAF8F2]">
                        ...
                        <td className="py-3 font-bold text-[#2E7D32]">
                          {centre.activeCounters} Digital Lanes
                        </td>
                        <td className="py-3 font-bold">
                          {centre.todayBooked} / {centre.todayCapacity}
                        </td>
                        <td className="py-3">
                          <div className="flex items-center gap-2">
          ...
          <div className="lg:col-span-4 space-y-4">
            <div className="bg-white rounded-3xl p-6 shadow-md border border-[#E0ECE0] space-y-3">
              ...
              {bottlenecks.slice(0, 2).map((b) => (
                <div
                  key={b.id}
                  className="p-3 bg-[#FAF8F2] rounded-2xl border border-amber-200 text-xs space-y-1"
                >
                  <div className="flex justify-between font-bold">
                    <span className="text-gray-900">{b.stage}</span>
                    <span className="text-red-600">
                      {b.currentMins} min (Exp: {b.expectedMins}m)
                    </span>
                  </div>
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-base font-bold text-gray-900">
                Explainable AI (XAI) Congestion Decomposition
              </h3>
            </div>
            <p className="text-xs text-gray-500 mt-0.5">
              Shapley-style factor breakdown explaining why current wait times
              are predicted at ~{estimatedWaitMins} mins
            </p>
          </div>
        ...
        <div className="bg-white rounded-3xl p-6 shadow-md border border-[#E0ECE0] space-y-6">
          <div>
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-base font-bold text-gray-900">
                Live Bottleneck Detection & Operator Dispatch
              </h3>
            </div>
            <p className="text-xs text-gray-500 mt-0.5">
              Automated comparison of expected vs actual cycle times across
              Quality Check, Weighing, and Procurement
            </p>
          </div>
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                  <div>
                    ...
                    <div className="flex gap-4 text-xs text-gray-600 mt-1">
                      <span>
                        Expected Time: <strong>{b.expectedMins} min</strong>
                      </span>
                      <span>
        ...
        <div className="bg-white rounded-3xl p-6 shadow-md border border-[#E0ECE0] space-y-6">
          <div>
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-base font-bold text-gray-900">
                Fraud Screening & Anomaly Engine
              </h3>
            </div>
            <p className="text-xs text-gray-500 mt-0.5">
              Automated detection of duplicate bookings, invalid workflow hops,
              and impossible timestamp gaps
            </p>
          </div>
              ...
              >
                ...
                <div className="flex items-center justify-between pt-2 border-t border-gray-100 text-xs">
                  ...
                  <div className="flex gap-2">
                    <button
                      onClick={() =>
                        resolveFraudAlert(fa.id, "Dismissed (False Positive)")
                      }
                    ...
                    <button
                      onClick={() =>
                        resolveFraudAlert(fa.id, "Flagged & Verified")
                      }
